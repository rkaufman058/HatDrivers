...
#        AWG.addwaveform(sequence_index, 4, block[i][2][1]+'_Q')
        ...
        AWG.addwaveform(sequence_index, 1, block[2][0+int(i/2)]+'_I')
        AWG.addwaveform(sequence_index, 2, block[2][0+int(i/2)]+'_Q')
        AWG.addwaveform(sequence_index, 3, block[2][1+int(i/2)]+'_I')
        AWG.addwaveform(sequence_index, 4, block[2][1+int(i/2)]+'_Q')        
        i = i + 4
...
class Sequence(object):
    def __init__(self, shotsnum, totwavenum, pulse):
        ...
        self.sequence_block = [[] for x in range(shotsnum-1)] # The length of this sequence_block should be shotsnum - 1
...
class SequenceBlock(object):
    ...
    def compare_old(self):
        count  = 0 
        compare_index = 0
        self.header = [[] for x in range(2*len(self.block))]
        self.header[0] = [[],[],[],[]]
        ...
        while compare_index < len(self.block)-1:        
            if self.block[compare_index][2] >= self.block[compare_index+1][2]:
                ...
                self.header[count] = [[] for x in range(4)]
...
#        self.header = [x for x in self.header if x != [] ]
    ...
    def make_block(self):
        self.make_header()
        self.new_block = [[[], [], []] for x in range(len(self.header))]
